...
// A live search link, not a generated summary -- the rule that sizes and
// times the trade is mechanical and tested; what's driving the underlying
// market is not, so it's left to a real, current source rather than
// anything this app writes itself. Currency pairs search on their own
// ticker (EURUSD reads better than "Euro / US Dollar" as a query); everything
// else searches on its plain-English label.
function newsSearchUrl(code, label, assetClass) {
  const query = `${assetClass === "currencies" ? code : label} price`;
  return `https://news.google.com/search?q=${encodeURIComponent(query)}&hl=en-US&gl=US&ceid=US%3Aen`;
}
  ...
  return (
    <article className="border-b border-rule px-4 py-4">
      ...
      <p className="mb-3 text-xs text-muted">
        <a
          href={newsSearchUrl(s.code, LABEL_BY_CODE[s.code] || s.code, s.asset_class)}
          target="_blank"
          rel="noopener noreferrer"
          className="underline decoration-rule underline-offset-2"
        >
          Recent news for {LABEL_BY_CODE[s.code] || s.code} ↗
        </a>{" "}
        — context on what may be moving it, not part of the rule.
      </p>
    ...
    <div className="mx-auto w-full max-w-3xl px-0 py-6">
      ...
      <details id="how-to-read" className="border-b border-rule px-4 py-3 text-xs text-ink-2">
        ...
        <dl className="mt-3 space-y-3">
          ...
          <div>
            <dt className="font-bold text-ink">Recent news</dt>
            <dd className="mt-0.5">
              A live search link, not anything generated by this app. The
              rule that decides direction and size is mechanical and tested;
              what&apos;s actually driving a market day to day is not, so
              that question is left to a real, current source instead of a
              summary this page would have to be trusted to get right.
              Reading it is optional and changes nothing here — no click
              feeds back into the signal.
            </dd>
          </div>
        </dl>
      </details>
